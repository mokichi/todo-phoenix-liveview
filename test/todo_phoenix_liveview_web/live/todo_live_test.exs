defmodule TodoPhoenixLiveview.TodoLiveTest do
  ...
  defp create_task(_) do
    {:ok, task} = Todo.create_task(%{title: "Task 1", completed: false})
    %{task: task}
  end

  defp create_completed_task(_) do
    {:ok, task} = Todo.create_task(%{title: "Task 2", completed: true})
    %{completed_task: task}
  end

  describe "Index" do
    setup [:create_task, :create_completed_task]
